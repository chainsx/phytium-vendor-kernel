...
static int phytium_edac_device_add(struct phytium_edac *edac)
{
	...
	edac_dev = edac_device_alloc_ctl_info(
		sizeof(struct edac_device_ctl_info), "ras", 1, "soc", 1, 0,
...
{
	const struct ras_error_info *err_info = pe220x_ras_error[error_group];
...
{
	writeq(MISC0_CEC(0XFE),
	       edac->ras_base[error_group] + ERR_MISC0(error_id));
	writeq(GENMASK(31, 0),
	       edac->ras_base[error_group] + ERR_STATUS(error_id));
}
...
static int phytium_edac_probe(struct platform_device *pdev)
{
	...
	edac->ras_base = devm_kcalloc(&pdev->dev, 3, sizeof(*edac->ras_base),
				      GFP_KERNEL);
	if (!edac->ras_base) {
		return -ENOMEM;
		goto out;
	...
	for (i = 0; i < irq_cnt; i++) {
		...
		ret = devm_request_irq(&pdev->dev, irq, phytium_edac_isr,
				       IRQF_SHARED, EDAC_MOD_STR, edac);
		if (ret) {
