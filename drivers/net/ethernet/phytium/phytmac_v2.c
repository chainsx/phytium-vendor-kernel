...
static int phytmac_get_mac_addr(struct phytmac *pdata, u8 *addr)
{
	...
	memcpy(&para, pdata->msg_regs + PHYTMAC_MSG(index) + MSG_HDR_LEN, 8);
...
static int phytmac_mdio_data_read(struct phytmac *pdata, int mii_id, int regnum, int is_c45)
{
	u16 data;
...
static int phytmac_mdio_data_write(struct phytmac *pdata, int mii_id,
...
struct phytmac_hw_if phytmac_2p0_hw = {
	...
	.mdio_read = phytmac_mdio_data_read,
	.mdio_write = phytmac_mdio_data_write,
